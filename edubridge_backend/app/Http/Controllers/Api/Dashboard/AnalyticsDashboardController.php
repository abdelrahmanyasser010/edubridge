...
class AnalyticsDashboardController {
 public function earlyWarnings(DashboardEarlyWarningRequest $request, EarlyWarningCalculator $calculator): JsonResponse {
  ...
  $filters=$request->validated(); $min=(int)($filters['min_score']??1);
  ...
  $items=$students->map(function(Student $student)use($calculator){$risk=$calculator->calculate($student); return $risk+[
