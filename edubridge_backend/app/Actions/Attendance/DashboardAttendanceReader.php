...
class DashboardAttendanceReader
{
    ...
    public function daily(array $filters): array
    {
        ...
        if (($filters['section_id'] ?? null) !== null && $sectionIds === []) $sectionIds=[(int)$filters['section_id']];
        ...
        $students = Student::query()->where('status',Student::STATUS_ACTIVE)
            ...
            ->when($filters['q'] ?? null, fn($q,$v)=>$q->where(function($sq) use($v){ $sq->where('full_name','like','%'.$v.'%')->orWhere('admission_number','like','%'.$v.'%'); }))
        ...
        $items=$students->map(function(Student $student) use($sessionsBySection,$records): array {
            ...
            $periods=[]; $absent=0; $late=0; $excused=0; $present=0; $recorded=0;
            foreach($sessions as $session){
                $record=$records->get($session->id.':'.$student->id);
                $status=$record?->status ?? 'not_recorded';
                if($record) $recorded++;
                if($status===AttendanceRecord::STATUS_ABSENT) $absent++;
                elseif($status===AttendanceRecord::STATUS_LATE) $late++;
                elseif($status===AttendanceRecord::STATUS_EXCUSED) $excused++;
                elseif($status===AttendanceRecord::STATUS_PRESENT) $present++;
                $periods[]=[
    ...
    public function atRisk(array $filters): array
    {
        ...
        $rows=DB::connection('tenant')->table('students')
            ...
            ->when($filters['q'] ?? null, fn($q,$v)=>$q->where(function($sq) use($v){$sq->where('students.full_name','like','%'.$v.'%')->orWhere('students.admission_number','like','%'.$v.'%');}))
