...
class AssessmentPublishWorkflowTest extends TestCase
{
    ...
    protected function setUp(): void
    {
        ...
        $this->centralDatabase = $this->sqliteDatabasePath('grade-publish-central');
        $this->tenantDatabase = $this->sqliteDatabasePath('grade-publish-tenant');
    ...
    public function test_assessment_can_be_submitted_approved_published_and_locked(): void
    {
        $teacherToken = $this->loginAndReturnToken($this->teacherUser, 'publish-teacher-device', 'teacher');
        $assessmentId = $this->withBearerToken($teacherToken)
        ...
        $this->withBearerToken($teacherToken)
            ->putJson('/api/v1/teacher/assessments/'.$assessmentId.'/grades', [
                'entries' => [['student_id' => $this->studentId, 'score' => 45]],
            ])
            ->assertOk();

        $this->withBearerToken($teacherToken)
            ->postJson('/api/v1/teacher/assessments/'.$assessmentId.'/submit')
            ->assertOk()
            ->assertJsonPath('data.status', 'pending_approval');

        $adminToken = $this->loginAndReturnToken($this->adminUser, 'publish-admin-device', 'dashboard');

        $this->withBearerToken($adminToken)
            ->postJson('/api/v1/assessments/'.$assessmentId.'/approve')
            ->assertOk()
        ...
        $this->withBearerToken($adminToken)
            ...
            ->assertOk()
            ->assertJsonPath('data.published_at', fn (?string $value): bool => $value !== null);

        $this->assertDatabaseHas('notifications', ['type' => 'assessment.published'], 'tenant');
        $this->assertDatabaseHas('notification_deliveries', ['central_user_id' => $this->parentUser->id, 'channel' => 'database'], 'tenant');
        $this->assertDatabaseHas('audit_logs', ['action' => 'assessment.published', 'subject_id' => $assessmentId], 'tenant');

        $this->withBearerToken($adminToken)
            ->postJson('/api/v1/assessments/'.$assessmentId.'/lock')
            ->assertOk()
            ->assertJsonPath('data.status', 'locked');

        $this->withBearerToken($teacherToken)
            ->putJson('/api/v1/teacher/assessments/'.$assessmentId.'/grades', [
                'entries' => [['student_id' => $this->studentId, 'score' => 44, 'revision' => 1]],
            ])
            ->assertConflict();
    }
    ...
    {
        $this->teacherUser = $this->createUser('Teacher', 'publish-teacher@example.test');
        $this->adminUser = $this->createUser('Academic Admin', 'publish-admin@example.test');
        $this->parentUser = $this->createUser('Parent', 'publish-parent@example.test');
        ...
        foreach ([[$this->teacherUser, 'teacher'], [$this->adminUser, 'academic_admin'], [$this->parentUser, 'parent']] as [$user, $role]) {
    ...
    private function seedSchoolData(): void
    {
        ...
        $teacherId = (int) DB::connection('tenant')->table('teachers')->insertGetId(['central_user_id' => $this->teacherUser->id, 'employee_number' => 'T-PUB-001', 'full_name' => 'Teacher', 'status' => 'active', 'created_at' => now(), 'updated_at' => now()]);
        $this->allocationId = (int) DB::connection('tenant')->table('teacher_section_subject')->insertGetId(['academic_term_id' => $termId, 'teacher_id' => $teacherId, 'section_id' => $sectionId, 'subject_id' => $subjectId, 'weekly_quota' => 5, 'is_homeroom' => false, 'status' => 'active', 'created_at' => now(), 'updated_at' => now()]);
        ...
        $this->studentId = (int) DB::connection('tenant')->table('students')->insertGetId(['admission_number' => 'S-PUB-001', 'full_name' => 'Student', 'grade_level_id' => $gradeLevelId, 'section_id' => $sectionId, 'status' => 'active', 'created_at' => now(), 'updated_at' => now()]);
