...
import { Sparkles, TrendingUp, TrendingDown, AlertTriangle, ShieldCheck, Zap, BarChart3, Users, BookOpen, CheckCircle, ArrowUpRight, ClipboardCheck, FileText, UserCheck } from "lucide-react";

export default function AnalyticsPage() {
  const { students, teachers, sections, behaviorNotes, issueParentSummons, showToast, apiStatus, dashboardSummary } = useDashboard();
  ...
  const avgAcademic = Math.round(students.reduce((a, b) => a + b.academicScore, 0) / (students.length || 1));
  const avgAttendance = Math.round(students.reduce((a, b) => a + b.attendanceRate, 0) / (students.length || 1));
  ...
  const getEducationalDiagnosis = (stu: Student) => {
    if (stu.riskLevel === "high") {
      return "انخفاض في درجات التقويم المستمر (<70%) متزامن مع غياب متكرر ورصد ملاحظة في دفتر المتابعة. يُوصى بإحالة الطالب للموجه الطلابي واستدعاء ولي الأمر للحضور.";
    }
    return "تذبذب في درجات المشاركة والواجبات مع تكرار التأخر الصباحي هذا الأسبوع. يُوصى بمتابعة مربي الفصل وإشعار ولي الأمر للتنبيه المسبق.";
  };
        ...
        <main className="page-body">
          ...
          {apiStatus === "live" ? (
            <div style={{ display: "grid", gap: 18 }}>
              <div className="card" style={{ padding: "18px 20px", borderRight: "4px solid var(--primary)" }}>
                <div style={{ fontWeight: 900, fontSize: 16, color: "var(--text-dark)", marginBottom: 6 }}>تحليلات Dashboard المتاحة من الخادم</div>
                <div style={{ fontSize: 12.5, color: "var(--text-light)", lineHeight: 1.8 }}>تم إيقاف مؤشرات الإنذار المبكر وKPI التي كانت محسوبة من Mock Data. الـ Backend الحالي لا يعيد معدل الطالب التراكمي أو حضوره التراكمي أو Risk Score لكل طالب، لذلك لن تعرض اللوحة أرقاماً مصطنعة.</div>
              </div>
              <div className="kpi-grid">
                {[
                  { label: "الطلاب", value: dashboardSummary?.students ?? students.length, icon: <Users size={22} />, bg: "#EFF6FF", color: "#1D4ED8" },
                  { label: "المعلمون", value: dashboardSummary?.teachers ?? teachers.length, icon: <UserCheck size={22} />, bg: "#F0FDF4", color: "#15803D" },
                  { label: "الشعب", value: dashboardSummary?.sections ?? sections.length, icon: <BookOpen size={22} />, bg: "#FFF7ED", color: "#C2410C" },
                  { label: "حضور اليوم", value: `${Math.round(dashboardSummary?.attendance_today?.rate ?? 0)}%`, icon: <ClipboardCheck size={22} />, bg: "#F5F3FF", color: "#7C3AED" },
                ].map((item) => (
                  <div key={item.label} className="kpi-card" style={{ cursor: "default" }}>
                    <div className="kpi-icon" style={{ background: item.bg, color: item.color }}>{item.icon}</div>
                    <div className="kpi-content"><div className="kpi-value" style={{ fontSize: 20 }}>{item.value}</div><div className="kpi-label">{item.label}</div></div>
                  </div>
                ))}
              </div>
              <div className="card">
                <div className="card-header"><div><div className="card-title">الطلبات المعلقة</div><div className="card-subtitle">قيم حقيقية من Dashboard Summary API</div></div></div>
                <div className="card-body" style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(160px,1fr))", gap: 12 }}>
                  {[
                    ["ملاحظات سلوكية", dashboardSummary?.pending?.behavior_notes ?? 0],
                    ["أعذار طبية", dashboardSummary?.pending?.medical_excuses ?? 0],
                    ["اعتراضات درجات", dashboardSummary?.pending?.grade_appeals ?? 0],
                    ["تصاريح خروج", dashboardSummary?.pending?.leave_permits ?? 0],
                  ].map(([label, value]) => <div key={String(label)} style={{ background: "var(--bg-page)", borderRadius: 12, padding: 16, textAlign: "center" }}><div style={{ fontSize: 22, fontWeight: 900 }}>{value}</div><div style={{ fontSize: 12, color: "var(--text-muted)" }}>{label}</div></div>)}
                </div>
              </div>
            </div>
          ) : (<>
          ...
          <div style={{
            ...
            <div style={{ zIndex: 1, maxWidth: 540 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 12, color: "rgba(255,255,255,0.8)" }}>تحديث آلي مباشر بالربط مع رصد المعلمين</span>
              </div>
              ...
              <div style={{ fontSize: 13.5, color: "rgba(255,255,255,0.85)", lineHeight: 1.6 }}>
                يقوم النظام بالربط التلقائي بين 4 عناصر أساسية (المعدل التراكمي للدرجات، الالتزام بالحضور الصباحي، السلوك في الفصول، والتواصل مع أولياء الأمور) لتقييم استقرار المدرسة والاكتشاف المبكر لحالات التعثر.
              </div>
            </div>
            ...
            <div style={{
              ...
              <div style={{ fontSize: 11, color: "rgba(255,255,255,0.7)", marginTop: 6 }}>نمو +2.4% مقارنة بالشهر السابق</div>
            </div>
          </div>
          ...
          <div className="kpi-grid" style={{ marginBottom: 24 }}>
            {[
              { label: "المعدل الأكاديمي العام", value: `${avgAcademic}%`, icon: <BookOpen size={22} />, bg: "#EFF6FF", color: "#1D4ED8", sub: "أعلى بنسبة 4% من المتوسط العام" },
              { label: "نسبة الالتزام بالحضور", value: `${avgAttendance}%`, icon: <ShieldCheck size={22} />, bg: "#F0FDF4", color: "#15803D", sub: "معدل الغياب اليومي مستقر الطبيعي (<4%)" },
              { label: "حالات الإنذار المبكر للمتابعة", value: `${riskStudents.length} طلاب`, icon: <AlertTriangle size={22} />, bg: "#FEF2F2", color: "#DC2626", sub: "طلاب تحت المتابعة والملاحظة الدورية" },
              { label: "كفاءة أنصبتة المعلمين", value: "96.4%", icon: <Zap size={22} />, bg: "#FFF7ED", color: "#C2410C", sub: "توزيع مثالي لحصص الانتظار والجدول" },
            ].map((st, idx) => (
            ...
            <div className="card">
              <div className="card-header">
                <div>
                  ...
                  <div className="card-subtitle">الطلاب الذين أظهر الرصد الدوري انخفاضاً في تحصيلهم عند ربط (الدرجات + الغياب + السلوك)</div>
                </div>
                <span className="badge badge-red"><span className="dot" />تحديث آلي بالربط مع نظام الرصد</span>
              </div>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                            ...
                            <span className={`badge ${isHigh ? "badge-red" : "badge-orange"}`}>
                              <span className="dot" /> {isHigh ? "خطر عالي (يحتاج إرشاد فوري)" : "متابعة متوسطة"}
                            </span>
                          ...
                          <div style={{ display: "flex", gap: 16, fontSize: 12, color: "var(--text-muted)", flexWrap: "wrap" }}>
                            <span>المعدل الدراسي: <strong style={{ color: isHigh ? "#DC2626" : "#EA580C" }}>{stu.academicScore}%</strong></span>
                            <span>نسبة الحضور: <strong style={{ color: stu.attendanceRate < 90 ? "#DC2626" : "#15803D" }}>{stu.attendanceRate}%</strong></span>
                            <span>ولي الأمر: <strong>{stu.parentName}</strong> (مرتبط بالتطبيق ✓)</span>
                          </div>
                        </div>
                      </div>
              ...
              <div style={{ padding: "20px" }}>
                <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: 18 }}>
                  {sections.map((sec, idx) => {
                    ...
                    const secAvg = Math.round(secStus.reduce((a, b) => a + b.academicScore, 0) / (secStus.length || 1));
                    const attAvg = Math.round(secStus.reduce((a, b) => a + b.attendanceRate, 0) / (secStus.length || 1));
                      ...
                      <div key={sec.id} style={{ background: "var(--bg-page)", padding: "16px 20px", borderRadius: "var(--radius)", border: "1px solid var(--border-light)" }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                          <div>
                            ...
                            <span style={{ fontSize: 12, color: "var(--text-muted)", marginRight: 10 }}>مربي الفصل: {sec.classTeacherName} ({sec.enrolledCount} طالب)</span>
                          </div>
                          <div style={{ display: "flex", gap: 14 }}>
                            <span style={{ fontSize: 13, fontWeight: 700, color: "var(--text-dark)" }}>متوسط الدرجات: <strong style={{ color: barColor }}>{secAvg}%</strong></span>
          ...
          {activeTab === "teachers" && (
            <div className="card">
              <div className="card-header">
                <div>
                  <div className="card-title">مؤشرات الأداء والكفاءة التشغيلية للكوادر التعليمية (Teacher KPIs)</div>
                  <div className="card-subtitle">تقييم سرعة رصد الغياب اليومي، دقة إدخال الدرجات، ومعدل إسناد حصص الانتظار</div>
                </div>
              </div>
              <div className="data-table-wrap">
                <table className="data-table">
                  ...
                  <tbody>
                    {teachers.map((tch) => {
                      const kpiColor = tch.kpiScore >= 95 ? "var(--green)" : tch.kpiScore >= 85 ? "var(--primary)" : "var(--warning)";
                      return (
                        <tr key={tch.id}>
                          <td>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                              ...
                              <div>
                                ...
                                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{tch.specialization}</div>
                              </div>
                            </div>
                          </td>
                          <td>
                            <span className="badge badge-gray" style={{ fontWeight: 700 }}>{tch.lessonsThisWeek} حصة / أسبوعياً</span>
                          </td>
                          ...
                          <td>
                            <span style={{ fontSize: 15, fontWeight: 900, color: kpiColor }}>{tch.kpiScore}%</span>
                          </td>
                          <td>
